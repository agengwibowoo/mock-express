...
const users = [
  {
    id: 1,
    username: 'demo',
    password: '$2a$10$5h3tRO3AxUCZ0B0tUOUcgOcWBjc03Pa2.RKE2buj0d7lBUx8iSWy.', // hashed "password123"
  ...
  res.json({
    ...
    endpoints: {
      ...
      protected: {
        ...
        employeePerformance: 'GET /api/protected/employee-performance (requires JWT)'
      },
      public: {
...
app.get('/api/protected/dashboard', authMiddleware, (req, res) => {
  res.json({
    ...
    data: {
      ...
      recentActivity: [
        { id: 1, action: 'Product viewed', timestamp: new Date().toISOString() },
        { id: 2, action: 'Login successful', timestamp: new Date().toISOString() }
      ]
    }
  });
});
...
app.get('/api/protected/employee-performance', authMiddleware, (req, res) => {
  ...
  if (emp_id) {
    filteredData = filteredData.filter(e => 
      e.emp_id.toLowerCase() === emp_id.toLowerCase()
    );
  }
  ...
  if (pos_code) {
    filteredData = filteredData.filter(e => 
      e.pos_code.toLowerCase() === pos_code.toLowerCase()
    );
  }
  
  // Filter by minimum achievement
  if (minAchievement) {
    filteredData = filteredData.filter(e => e.achievement >= parseInt(minAchievement));
  }
